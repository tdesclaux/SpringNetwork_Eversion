...
r_save = []
folder = r"./results/"   # chemin du dossier
filename_r = "data2DRessortsTorsion_Naxial=10_Ntheta=60_Nnetwork=2_B=0.001_k=1.0.csv"                             # nom du fichier
filename_s = "springs_array2DRessortsTorsion_Naxial=10_Ntheta=60_Nnetwork=2_B=0.001_k=1.0.csv"                             # nom du fichier
filename_b = "bending_springs_array2DRessortsTorsion_Naxial=10_Ntheta=60_Nnetwork=2_B=0.001_k=1.0.csv"                             # nom du fichier
filepath = os.path.join(folder, filename_r)
...
interaction_steps = 500  # Nombre d'étapes pour la phase d'interaction
total_steps = initiation_steps + interaction_steps
step2save = np.arange(-1,total_steps,1e2)
step2save[0]=0
...
if (redefine_springs==1):
    springs=[]
    for inetwork_demande in range(Nnetwork):
        for iz in range(Ndr):
            for iy in range(Ntheta):
                for ix in range(Naxial):
                    i = node_index(ix, iy, iz,inetwork_demande,Naxial,Ntheta,Ndr,Nnetwork)
                    for dx in [-1, 0, 1]:
                        for dy in [-1, 0, 1]:
                            for dz in [-1, 0, 1]:
                                if dx == dy == dz == 0:
                                ...
                                if not (0 <= jx < Naxial+1 and 0 <= jz < Ndr):
                                    continue
                                if jx == Naxial and inetwork_demande==0 :
                                    inetwork_vu=1
                                    jx=0
                                if jx == Naxial and inetwork_demande==1 :
                                    continue

                                
                                j = node_index(jx, jy, jz,inetwork_vu,Naxial,Ntheta,Ndr,Nnetwork)
    ...
    springs_array = np.loadtxt(folder + "/" + filename_s, delimiter=",")
...
if (redefine_springs==1):
    ...
    bending_springs_array = np.loadtxt(folder + "/" + filename_b, delimiter=",")
...
str_args = "2DRessortsTorsion_Naxial=" + str(Naxial) + "_Ntheta=" +str(Ntheta) +"_B=" + str(B) + "_k=" + str(k1) + "_restart"
path = r"./results"
...
for i_time in range(0,len(step2save),1):
    ...
    for slice_number in  range(Naxial):
        # slice_number=0
        for inetwork in range(Nnetwork):
