...
for B in [0.05,0.1,0.5]:
    ...
    for step in range(15,20):
        ...
        if step == 15:
            plt.plot(x[IDX_PLOT], z[IDX_PLOT]/P_save[step]*B, '-',color=colB[ib],label=f'B={B}')
        else:
            plt.plot(x[IDX_PLOT], z[IDX_PLOT]/P_save[step]*B, '-',color=colB[ib])
        # plt.axis('equal')
        plt.xlabel('x (m)',fontsize=20); plt.ylabel('$u_z B/P$ (m$^5$)',fontsize=20)
