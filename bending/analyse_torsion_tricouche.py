...
for step in range(15,n_step):
    ...
    plt.plot(x[IDX_PLOT], z[IDX_PLOT]/P_save[step]*dZ*dZ*dZ, '-',color=[step/n_step,0,0],label=f'déformé incrément={step}')
    # plt.axis('equal')
...
# plt.figure()
# plt.plot(x[IDX_PLOT], z[IDX_PLOT], '-',color=[1,0,0],label=f'déformé incrément={step}')
# plt.plot(x[IDX_PLOT2], z[IDX_PLOT2], '-',color=[0,0,0],label=f'déformé incrément={step}')
# plt.plot(x[IDX_PLOT3], z[IDX_PLOT3], '-',color=[0,0,1],label=f'déformé incrément={step}')

# # plt.axis('equal')
# plt.xlabel('x'); plt.ylabel('z')
# plt.grid(True, alpha=0.3)
# X_theo = np.linspace(-1,NX-1)
# B = dZ**3
# z=np.array(F_save[-1])/24/B*X_theo**2*(6*NX**2-4*NX*X_theo+X_theo**2)
# plt.plot(X_theo+0.5,z/6+1)


# plt.figure()
# plt.plot(delta,F_save)
# # Fait la théorie
# delta_theo = np.array(F_save)/B/8*(8**4)/100
# plt.plot(delta_theo,F_save)
# plt.grid(True, alpha=0.3)
